...
CREDIT="\u00a9 Hope A Little"   # quiet ownership mark on the photo itself
...
def credit(base, px, py, pw, ph, W):
    """Quiet ownership mark in the photo's lower-right corner.

    Drawn on a transparent layer so the white can sit at low opacity over a
    soft dark shadow — that combination stays legible on bright sky and on
    dark forest without ever reading as loud.
    """
    f = italic(max(11, int(W*0.019)))
    pad = int(W*0.022)
    x, y = px + pw - pad, py + ph - pad
    layer = Image.new("RGBA", base.size, (0,0,0,0))
    dl = ImageDraw.Draw(layer)
    for ox, oy in ((1,1),(2,2)):
        dl.text((x+ox, y+oy), CREDIT, font=f, fill=(0,0,0,70), anchor="rs")
    dl.text((x, y), CREDIT, font=f, fill=(255,255,255,105), anchor="rs")
...
def render_still(photo_path, title, tagline, scripture, out_path, hero=HERO):
    ...
    base.paste(credit(base, (W-pw)//2, py, pw, phh, W), (0,0))
